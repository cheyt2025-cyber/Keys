-- ZAPORIUM HUB LOADER - FINAL VERSION 2025
-- Paste once → works forever until 24h expires
-- Auto save + auto delete after 24 hours
...
local SAVE_FILE = "ZaporiumKeySave.txt"  -- auto created + auto deleted

-- Check if key is valid via your server
...
-- Auto-delete saved key after 24 hours
local function scheduleKeyDelete()
    task.delay(24 * 60 * 60, function() -- 24 hours
        if isfile and isfile(SAVE_FILE) then
            delfile(SAVE_FILE)
            print("[Zaporium] 24-hour key expired → save file deleted")
        end
    end)
end
...
if savedKey and isKeyValid(savedKey) then
    -- Key still valid → skip GUI completely
    print("[Zaporium] Saved key accepted! Loading script instantly...")
    scheduleKeyDelete() -- restart 24h timer
else
    -- No valid saved key → show GUI and save when correct
    ZaporiumKeySystem.new({
        ...
        ValidateKey = function(key)
            ...
            if valid and writefile then
                ...
                print("[Zaporium] Key saved! You won't need to paste again for 24 hours.")
                scheduleKeyDelete()
            end
            return valid
        end,
